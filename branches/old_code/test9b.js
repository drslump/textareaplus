...
//TODO: The compound regexes are not being builded yet
//TODO: Add support to 'include' templates and other grammars
function parseDef( def, end, parent ) {

	var i, out, res, patterns;

	if (typeof def.name !== 'string') {
		out.names = def.name;
	} else {
		out.names = { 0: def.name };
	}
	
	out.captures = def.captures;
	...
	{		
		...
		// Make sure it's an object with an array
		if ( String(patterns.constructor).match(/\bArray\b/) ) {
			patterns = { 0: patterns };	
		}
		...
		for (n in patterns) { 
			...
			for (i=0; i<patterns[n].length; i++)
			{
				res.push(patterns[n][i].begin);
		
				out.patterns[n][i] = parseDef( patterns[n][i], def.end, out );
			}
		}
	}
	...
	patterns: [
		{
			...
			patterns: [
				{	//Single line Comments
					match	: /((#)|(\/\/)).*?$/,
					name	: {
						2: 'comment.line.number-sign.php',
						3: 'comment.line.double-slash.php'
					},
					captures: {
						1: 'punctuation.definition.comment.php',
					}
				},				
				{	//Multi line comments with support for PHPDOC
					begin	: /(\/\*(\*(?=\s*$))?)/,
					end		: /(\*\/)/,
					name	: {
						0: 'comment.block.php',
						2: 'comment.block.documentation.phpdoc.php'
					}
					captures: {
						1: 'punctuation.definition.comment.php'
					}
				...
				{	//Double quote strings
					...
					patterns: [
						...
						'#ComplexParsingVariable',
						'#SimpleParsingVariable',
					]
				},
				{	//Heredoc strings
				...
				{	//Numbers
					match	: /[+-]?(?:(?:0([Xx][0-9A-Fa-f]+)|([0-7]+))|([0-9]*\.[0-9]+[Ee]?[+-]?[0-9]*)|([0-9]+))/,
					name	: {
						0: 'constant.numeric.php',
						1: 'constant.numeric.hex.php',
						2: 'constant.numeric.octal.php',
						3: 'constant.numeric.float.php',
						4: 'constant.numeric.integer.php'
					},
				},
				{	// Operators
					match	: /[\!\=]\={0,2}|\|\||\&\&|\+\+|\-\-|[\+\-\*\/\.\&\|\%\^]\=?|\>(?:\>\=|\>|\=)?|\<(?:\<\=|\<|\=])?|\:{1,2}|\-\>|\?(?!>)|[@;]/,
				...
				{	// Idents
					match	: /\b[A-Za-z_\x7f-\xff]+[A-Za-z0-9_\x7f-\xff]*\b/,
					name	: 'keyword.ident.php'
				}
			]
		}
	],
